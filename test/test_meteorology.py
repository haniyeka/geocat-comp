...
class Test_dewtemp(unittest.TestCase):
    ...
    def test_list_input(self, use_gpu = False):
        ...
        assert np.allclose(dewtemp(tk, self.rh_def, use_gpu) - 273.15, self.dt_2, 0.1)
    ...
    def test_dims_error(self, use_gpu = False):
        self.assertRaises(ValueError, dewtemp, self.t_def[:10], self.rh_def[:8], use_gpu)
    ...
    def test_dask_compute(self, use_gpu = False):
        ...
        assert np.allclose((dewtemp(tk, rh, use_gpu) - 273.15).data, self.dt_2, atol=0.1)

    def test_dask_lazy(self, use_gpu = False):
        tk = xr.DataArray(np.asarray(self.t_def) + 273.15).chunk(6)
        rh = xr.DataArray(self.rh_def).chunk(6)

        assert isinstance((dewtemp(tk, rh, use_gpu) - 273.15).data, dask.array.Array)
...
class Test_heat_index(unittest.TestCase):
    ...
    def test_numpy_input(self, use_gpu = False):
        assert np.allclose(heat_index(self.t1, self.rh1, False, use_gpu = use_gpu),
                           self.ncl_gt_1,
                           atol=0.005)

    def test_multi_dimensional_input(self, use_gpu = False):
        assert np.allclose(heat_index(self.t2.reshape(2, 5),
                                      self.rh2.reshape(2, 5), True, use_gpu = use_gpu),
                           np.asarray(self.ncl_gt_2).reshape(2, 5),
                           atol=0.005)

    def test_alt_coef(self, use_gpu = False):
        assert np.allclose(heat_index(self.t2, self.rh2, True, use_gpu = use_gpu),
                           self.ncl_gt_2,
                           atol=0.005)

    def test_xarray_alt_coef(self, use_gpu = False):
        assert np.allclose(heat_index(xr.DataArray(self.t2),
                                      xr.DataArray(self.rh2), True, use_gpu = use_gpu),
                           self.ncl_gt_2,
                           atol=0.005)

    def test_float_input(self, use_gpu = False):
        assert np.allclose(heat_index(80, 75, use_gpu = use_gpu), 83.5751, atol=0.005)
    ...
    def test_xarray_input(self, use_gpu = False):
        ...
        assert np.allclose(heat_index(t, rh, use_gpu = use_gpu), self.ncl_gt_1, atol=0.005)

    def test_alternate_xarray_tag(self, use_gpu = False):
        t = xr.DataArray([15, 20])
        rh = xr.DataArray([15, 20])

        out = heat_index(t, rh, use_gpu = use_gpu)
        assert out.tag == "NCL: heat_index_nws; (Steadman+t)*0.5"

    def test_rh_warning(self, use_gpu = False):
        self.assertWarns(UserWarning, heat_index, [50, 80, 90], [0.1, 0.2, 0.5], use_gpu = use_gpu)

    def test_rh_valid(self, use_gpu = False):
        self.assertRaises(ValueError, heat_index, [50, 80, 90], [-1, 101, 50], use_gpu = use_gpu)
    ...
    def test_xarray_rh_valid(self, use_gpu = False):
        self.assertRaises(ValueError, heat_index, xr.DataArray([50, 80, 90]),
                          xr.DataArray([-1, 101, 50]), use_gpu = use_gpu)

    def test_xarray_type_error(self, use_gpu = False):
        self.assertRaises(TypeError, heat_index, self.t1,
                          xr.DataArray(self.rh1), use_gpu = use_gpu)

    def test_dims_error(self, use_gpu = False):
        self.assertRaises(ValueError, heat_index, self.t1[:10], self.rh1[:8], use_gpu = use_gpu)
    ...
    def test_dask_lazy(self, use_gpu = False):
        ...
        assert isinstance((heat_index(t, rh, use_gpu = use_gpu)).data, dask.array.Array)
